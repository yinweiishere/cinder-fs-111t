...
class AwsEc2VolumeDriver(driver.VolumeDriver):
    ...
    def _get_provider_volume(self, volume_id):
        ...
        try:
            #if not provider_volume_id:
            provider_volumes = self.adpter.list_volumes(ex_filters={'tag:hybrid_cloud_volume_id':volume_id})
...
#                                            origin_device_name)
    ...
    def copy_image_to_volume(self, context, volume, image_service, image_id):
        ...
        retry_time = 10
        provider_volume=self._get_provider_volume(provider_volume.id)
        while retry_time > 0:
            if provider_volume and provider_volume.state == StorageVolumeState.AVAILABLE:
                break
            else:
        ...
        try:
            ...
            conn=rpyc.connect(self.configuration.cgw_host_ip,'1111')
            copy_file_to_device_result = conn.root.copy_volume_to_file(image_id,device_name)
            if not copy_file_to_device_result:
                LOG.error("qemu-img convert %s %s failed" %(image_id,device_name)) 
                self.adpter.detach_volume(provider_volume)
                self.adpter.destroy_volume(provider_volume)
